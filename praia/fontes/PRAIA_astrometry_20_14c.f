...
c
c
c     Nesta versao, as imagens-traco presentes no campo sao identificadas automaticamente.
c
c
c      Last update: Marcelo Assafin - 15 Agosto 2015
c   
c
c
     ...
     ?iu4i=1440,mpes=50000,nhist=10,nhisfw=10,jfdp=10000,kfund=150000)
     ...
     ?nval(idiobs),ior1(idiobs),nval1(idiobs)
...
c     open (1,file='PRAIA_astrometry_20_13.dat')
...
c
c     Incializa a matriz que Marca pixels a serem excluidos no processo
c     de identificacao de objetos
c
...
c
      ...
      call ident (ipmax,idiobs,pixmat,imagem,nx,ny,fatceu,lper,iflag,
     ?mazi,ior,nval,xid,yid,idx1,idx2,idy1,idy2,idlado,npix,bcg,nstar)
...
c 
c
C      Last update:   15/08/2015 - Marcelo Assafin
C
C
...
c
c
c     Last modified: 07 Oct 2014
c
c
c


      subroutine ident (ipmax,idiobs,pixmat,imagem,nx,ny,fatceu,lper,
     ?iflag,mazi,ior,nval,xid,yid,idx1,idx2,idy1,idy2,idlado,npix,bcg,
     ?nest)
...
c
      ...
      if (pixmat(j,i).ge.mazi) go to 45
...
c
      ...
      if (pixmat(jj,ii).ge.mazi) go to 22
...
c     write (*,*) 'j,i,jm,im,ilax,ilay,lper= ',j,i,jm,im,ilax,ilay,lper
      ...
      call sperim (ipmax,idiobs,nx,ny,mazi,jm,im,ilax,ilay,lper,pixmat,
     ?contag,n,jx1,jx2,jy1,jy2)
...
c
c     Computes the typical count value of the inner perimeter by the mode
c     of the histogram of counts of the perimeter; also computes average
...
c     write (*,*) '5'
      ...
      call  bgstat (idiobs,n,contag,xm1,xs1)
...
c     write (*,*) '7'
      ...
      call sperim (ipmax,idiobs,nx,ny,mazi,jm,im,jlax,jlay,lper,pixmat,
     ?contag,n,jx1,jx2,jy1,jy2)
...
c     mode of the histogram of counts of the extended perimeter; also
c     computes average and standard deviation values.
c

c     write (*,*) '11'

      call  bgstat (idiobs,n,contag,xm2,xs2)
...
c
      ...
      if (pixmat(jj,ii).ge.mazi) go to 24
...
c
 ...
 25   xc=0.d0
      ...
      if (pixmat(jj,ii).ge.mazi) go to 27
...
c
      ...
      if (pixmat(jj,ii).ge.mazi) go to 30
...
c     do 43  nn=1,nstar
c
c     if (lado.le.idlado(nn)) then
c
c     if (xc.lt.idx1(nn)) go to 43
c     if (yc.lt.idy1(nn)) go to 43
c     if (xc.gt.idx2(nn)) go to 43
c     if (yc.gt.idy2(nn)) go to 43
c
c     go to 45
c
c     else
c
c     if (xid(nn).lt.ix1) go to 43
c     if (yid(nn).lt.iy1) go to 43
c     if (xid(nn).gt.ix2) go to 43
c     if (yid(nn).gt.iy2) go to 43
c
c     iflag(nn)=1
c
c
...
c
      ...
      subroutine bgstat (idiobs,n,contag,xm,xs)
...
c     mazi            = cutoff limit for counts from pixmat
...
c
      ...
      subroutine sperim (ipmax,idiobs,nx,ny,mazi,jm,im,ilax,ilay,lper,
     ?pixmat,contag,n,jx1,jx2,jy1,jy2)
...
c
      ...
      irmax2=(ilax+lper-1)**2+(ilay+lper-1)**2
...
c
      ...
      if (pixmat(jj,ii).ge.mazi) go to 10
