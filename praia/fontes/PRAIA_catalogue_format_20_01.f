...
c     in the internal PRAIA format. Catalogues with the internal PRAIA format
c     can be used by the PRAIA astrometric task for (RA,Dec) reductions.
c
c
...
c
c
c     PRAIA__catalogue_format < PRAIA__catalogue_format.dat > PRAIA_catalogue_format.log
c
c
c
c     Last update: M. Assafin - 16/Nov/2012
c
c
...
c
c     Writes output catalog in PRAIA format
c
...
c
c     (RA,DEc) in hexagesimal notation
c
      ...
      do k=1,7
      if (isa(k:k).eq.' ') isa(k:k)='0'
      enddo

      do k=1,6
      if (isd(k:k).eq.' ') isd(k:k)='0'
      enddo
...
c
       ...
       ex=dsqrt(coex(i)*coex(i)+cerau(i)*cerau(i)+0.02d0*0.02d0)
       ey=dsqrt(coey(i)*coey(i)+cedeu(i)*cedeu(i)+0.02d0*0.02d0)


c
c     Proper motion error estimates (no estimates for now)
c
      ...
      write (7,20) iah,iam,isa,isig,idg,idm,isd,ex,ey,codj(i),copma(i),
     ?copmd(i),erpma,erpmd,codmg(i),res2mg(i)




 20   format(i2.2,1x,i2.2,1x,a7,2x,
     ?a1,i2.2,1x,i2.2,1x,a6,2x,2f7.3,2x,f16.8,1x,
     ?4(1x,f7.3),2(1x,f6.3))
