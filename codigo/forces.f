!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      use constants
!      use variables
      Real (kind=wp) x(90000), v(90000), a(90000), a1(90000), r0(30000), tm, rij, a2(90000), d(3), vip, tt, td, ff1(3), ff2(3)
     

!cccccc      force of the central body
      r0 = (/ (SQRT(dot_product(x(3*i-2:3*i),x(3*i-2:3*i))), i=1,nbody) /)  ! calculates module for each vector
...
!            print *, 'rij', rij
            ff1 = Gaussk*Gaussk*mass(j)*d/(rij**3)
!            print *, 'ff1', ff1
            ff2 = Gaussk*Gaussk*mass(j)*(x(3*j-2:3*j)/(r0(j)**3))
!            print *, 'ff2', ff2
            a2(3*i-2:3*i) = a2(3*i-2:3*i) + ff1 - ff2
...
!      print *, 'mtot', mtot
      vel = (/ (sqrt(dot_product(v(3*i-2:3*i),v(3*i-2:3*i))), i=1,nsat) /) ! calculates module for each vector
      encin1 = sum(mass(1:nsat)*vel(1:nsat)*vel(1:nsat))/2.e0_wp
      
!      print *, 'encin1', encin1

      encin2 = sum(mass(1:nsat)*vel(1:nsat))
      encin2 = (encin2*encin2)/(2.e0_wp*Mtot)
!      print *, 'encin2', encin2

      r0 = (/ (SQRT(dot_product(x(3*i-2:3*i),x(3*i-2:3*i))), i=1,nsat) /)
      enpot1 = sum(GaussK*GaussK*mass0*mass(1:nsat)/r0(1:nsat))
!      print *, 'enpot1', enpot1
