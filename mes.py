...
def readxy(f):
    for i in ['ucac4.red.xy','2mass.red.xy']:
        arq = f[:-4]+i
        x, y, ra, dec, jd, s = np.loadtxt(arq, usecols=[0,1,33,34,41, 29], unpack=True)
        if s[0] > 2:
...
def proj(coord):
    a = np.sin(coord.dec)*np.sin(coord.dec[0])+np.cos(coord.dec)*np.cos(coord.dec[0])*np.cos(coord.ra-coord.ra[0])
    X = np.cos(coord.dec)*np.sin(coord.ra - coord.ra[0])/a
    Y = (np.sin(coord.dec)*np.cos(coord.dec[0])-np.cos(coord.dec)*np.sin(coord.dec[0])*np.cos(coord.ra-coord.ra[0]))/a
    return X,Y
    
def func(x, a, b, c):
    return a + b*x[0] + c*x[1]
    
def invert_func(coord, ajx, ajy):
    X, Y = proj(coord)
...
t = Time.now()
eph = read2('../target_Triton_160', coord_col=[0,1,2,3,4,5], time_col=[6], time_fmt='jd')
ephtri = {'coord': SkyCoord(eph['coords'][0], eph['coords'][1], unit=(u.hourangle, u.deg)), 'time': Time(eph['times'], format='jd', scale='utc')}
...
for i in dados.keys():
    ...
    k = np.where(c[n].separation(d[m]) < 200*u.mas)
    l = [x for x in np.arange(len(d)) if x not in k[1]]
    if len(l) == 0:
        continue
    np.concatenate((meancoordra, d[l].ra.value))
    np.concatenate((meancoorddec, d[l].dec.value))
