...
#    circle=plt.Circle((ax,by), tamanho,color='r')
#    fig.gca().add_artist(circle)
#    ang = np.arange(0,360, 1)*u.deg
#    argx = (tamanho/2)*np.cos(ang)
#    argy = (tamanho/2)*np.sin(ang)
#    m.contourf((ax + argx), by + argy, 1)
#    plt.plot(ax,by,60,color='r')
    m.plot(ax,by, 'o', axes=plt.gca(), color='red')
...
{}  {:02d} {:02d} {:07.4f} {:+02d} {:02d} {:06.3f} {:6.3f} {:6.2f} {:6.2f}  {:5.2f} {:5.1f} {:4.1f}  {:3.0f}'
    ...
    fig.savefig('{}_{}.png'.format(obj, datas[idx].isot),dpi=100)
...
tamanho = float(in_data[19].rsplit()[0])*u.km
