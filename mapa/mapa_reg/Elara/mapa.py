...
#    m = Basemap(projection='ortho',lat_0=center_map.latitude.value,lon_0=center_map.longitude.value,resolution=resolution)
    m = Basemap(projection='ortho',lat_0=center_map.latitude.value,lon_0=center_map.longitude.value,resolution=resolution,llcrnrx=-1500000.,llcrnry=1000000.,urcrnrx=3500000.,urcrnry=5130000.)
...
#    plt.legend(fontsize=mapsize[0].value*21/46)
    ...
    m.plot(7978218.83464, 9725527.7422, '+', color='black', markersize=mapsize[0].value*20/46)
#7978218.83464 9725527.7422
...
#    plt.xlabel(labelx, fontsize=mapsize[0].value*21/46, fontproperties='FreeMono', weight='bold')
    ...
    plt.xscale(u'linear')
    plt.xticks(np.arange(0, 5000000, 500000) + e - 1500000, np.arange(0.0, 5.0, 0.5), fontsize=sizel)
    plt.xlabel(r'$\times 10^{3}$(km)', fontsize=sizel)
    plt.yscale(u'linear')
    plt.yticks(np.arange(0, 4500000, 500000) + w + 1000000, np.arange(0.0, 4.5, 0.5), fontsize=sizel)#, rotation=90)
...
#        def callgeramapa(i, lats=None, erro=None, ring=None, atm=None, clat=None):
            ...
            geramapa(self.stars[i], self.datas_off[i], self.title[i], self.labelx[i], self.nameimg[i], mapstyle=self.mapstyle, resolution=self.resolution, centermap=None, lats=lats, erro=erro, ring=ring, atm=atm, clat=clat, sitearq=self.sitearq, fmt='eps', dpi=300, mapsize=self.mapsize, cpoints=cpoints, off=off)
