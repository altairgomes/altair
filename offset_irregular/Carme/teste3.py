...
resid = residuos(fun, par=ajdenwg.beta, x=x, y=eph[7])
f.write('Residuos: {} mas\n'.format(resid))

plt.plot(eph[0] - 2451544.5, eph[7], color='blue', label='Offsets')
plt.plot(eph[0] - 2451544.5, fun(ajdenwg.beta, x), color='red', label='Sem peso')
for i in eph[0][j]:
...
fig.savefig('Ajuste-x.png',dpi=200, bbox_inches='tight')
plt.clf()
...
ajranwg = least(func=fun, x=x, y=eph[8], beta0=beta0)
f.write('\nResultados do ajuste sem peso:\n')
for i in np.arange(len(ajranwg.beta)):
    f.write('p[{}]: {} \pm {}\n'.format(i, ajranwg.beta[i], ajranwg.sd_beta[i]))
resid = residuos(fun, par=ajranwg.beta, x=x, y=eph[8])
f.write('Residuos: {} mas\n'.format(resid))


plt.plot(eph[0] - 2451544.5, eph[8], color='blue', label='Offsets')
plt.plot(eph[0] - 2451544.5, fun(ajranwg.beta, x), color='red', label='Sem peso')
for i in eph[0][j]:
    plt.axvline(i - 2451544.5, color='black')
plt.xlabel('Tempo')
plt.xticks(r, ['{}'.format(i) for i in np.arange(1990,2021,1)], rotation=90)
...
fig =plt.gcf()
fig.set_size_inches((40.0*u.cm).to(u.imperial.inch).value,(16.0*u.cm).to(u.imperial.inch).value)
fig.savefig('Ajuste-y.png',dpi=200, bbox_inches='tight')
